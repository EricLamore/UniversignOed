...
export class HeaderConfigurationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.headerConfigurationService
      ...
      .subscribe(
        (res: HttpResponse<IHeaderConfiguration[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IHeaderConfiguration): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInHeaderConfigurations(): void {
    this.eventSubscriber = this.eventManager.subscribe('headerConfigurationListModification', () => this.loadPage());
  }

  delete(headerConfiguration: IHeaderConfiguration): void {
  ...
  protected onSuccess(data: IHeaderConfiguration[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
