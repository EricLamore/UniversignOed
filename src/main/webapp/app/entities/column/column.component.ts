...
export class ColumnComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.columnService
      ...
      .subscribe(
        (res: HttpResponse<IColumn[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IColumn): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInColumns(): void {
    this.eventSubscriber = this.eventManager.subscribe('columnListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IColumn[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
