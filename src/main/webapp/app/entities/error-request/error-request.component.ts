...
export class ErrorRequestComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.errorRequestService
      ...
      .subscribe(
        (res: HttpResponse<IErrorRequest[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IErrorRequest): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInErrorRequests(): void {
    this.eventSubscriber = this.eventManager.subscribe('errorRequestListModification', () => this.loadPage());
  }

  delete(errorRequest: IErrorRequest): void {
  ...
  protected onSuccess(data: IErrorRequest[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
