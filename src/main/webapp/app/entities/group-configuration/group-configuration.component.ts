...
export class GroupConfigurationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.groupConfigurationService
      ...
      .subscribe(
        (res: HttpResponse<IGroupConfiguration[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IGroupConfiguration): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInGroupConfigurations(): void {
    this.eventSubscriber = this.eventManager.subscribe('groupConfigurationListModification', () => this.loadPage());
  }

  delete(groupConfiguration: IGroupConfiguration): void {
  ...
  protected onSuccess(data: IGroupConfiguration[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
