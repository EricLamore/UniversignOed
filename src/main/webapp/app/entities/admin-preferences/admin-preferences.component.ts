...
export class AdminPreferencesComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.adminPreferencesService
      ...
      .subscribe(
        (res: HttpResponse<IAdminPreferences[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IAdminPreferences): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInAdminPreferences(): void {
    this.eventSubscriber = this.eventManager.subscribe('adminPreferencesListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAdminPreferences[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
