...
export class GroupPermissionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.groupPermissionService
      ...
      .subscribe(
        (res: HttpResponse<IGroupPermission[]>) => this.paginateGroupPermissions(res.body, res.headers),
  ...
  registerChangeInGroupPermissions() {
    this.eventSubscriber = this.eventManager.subscribe('groupPermissionListModification', response => this.loadAll());
  }
