...
export class OperatorService {
  ...
  protected convertDateFromClient(operator: IOperator): IOperator {
    const copy: IOperator = Object.assign({}, operator, {
      ...
      birthDate: operator.birthDate && operator.birthDate.isValid() ? operator.birthDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((operator: IOperator) => {
        ...
        operator.birthDate = operator.birthDate ? moment(operator.birthDate) : undefined;
      });
    }
    return res;
  }
